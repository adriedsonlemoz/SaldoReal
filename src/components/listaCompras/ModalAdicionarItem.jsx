    ...
    <Dialog fullScreen open={open} onClose={onClose} TransitionComponent={Transition} PaperProps={{ sx: { bgcolor: 'background.default', backgroundImage: 'none' } }}>
      <AppBar position="sticky" elevation={0} sx={{ background: 'linear-gradient(135deg,#2D0B5E 0%,#5A189A 58%,#7B2CBF 100%)', color: '#fff', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
        <Toolbar sx={{ minHeight: '62px !important', px: '14px !important' }}>
          <IconButton edge="start" onClick={onClose} aria-label="Fechar" sx={{ color: '#fff', width: 44, height: 44, bgcolor: 'rgba(255,255,255,.08)' }}><CloseIcon /></IconButton>
          ...
          <Button variant="contained" size="small" disabled={!podeSalvar || saving} onClick={handleAdicionar} startIcon={<AddShoppingCartIcon sx={{ fontSize: '18px !important' }} />} sx={{
            minHeight: 44, borderRadius: '12px', px: 1.45, fontWeight: 900,
      ...
      <DialogContent sx={{ px: { xs: 1.5, sm: 2.5 }, py: 1.8, pb: 2.5, maxWidth: 560, mx: 'auto', width: '100%' }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2, minmax(0,1fr))', sm: 'repeat(4, minmax(0,1fr))' }, gap: .8, mb: 1.25 }}>
          {GRUPOS_UNIDADE.map(g => { const ativo = grupoUnidade === g; const meta = GRUPO_META[g] || { icon: '•', descricao: '' }; return (
            <Box key={g} role="button" tabIndex={0} aria-pressed={ativo} onClick={() => handleGrupoChange(g)} onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && handleGrupoChange(g)} sx={{
              ...
              <Box sx={{ minWidth: 0 }}><Typography sx={{ fontSize: '.75rem', fontWeight: 900, color: ativo ? '#6A23A7' : 'text.primary', lineHeight: 1.05 }}>{g}</Typography><Typography sx={{ fontSize: '.66rem', color: 'text.secondary', mt: .15, lineHeight: 1.1 }}>{meta.descricao}</Typography></Box>
            </Box>
          ); })}
        </Box>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: .7 }}>
            {unidadesFiltradas.map(u => { const ativo = form.unidade === u.id; return (
              <Box key={u.id} role="button" tabIndex={0} aria-pressed={ativo} onClick={() => setForm(prev => ({ ...prev, unidade: u.id }))} onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && setForm(prev => ({ ...prev, unidade: u.id }))} sx={{
                ...
                <Typography sx={{ fontSize: '.66rem', fontWeight: 700, opacity: ativo ? .82 : .62, mt: .3, lineHeight: 1.05, textAlign: 'center' }}>{UNIDADE_DESC[u.id] || u.base}</Typography>
              </Box>
            ); })}
          </Box>
        </Box>
        ...
        {valorTotal > 0 && <Box sx={{ mt: 1.3, p: 1.35, borderRadius: '15px', background: 'linear-gradient(135deg, rgba(123,44,191,.08), rgba(247,37,133,.055))', border: '1px solid rgba(123,44,191,.16)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 1 }}>
          <Box sx={{ minWidth: 0 }}><Typography sx={{ fontSize: '.66rem', fontWeight: 900, color: '#6A23A7', textTransform: 'uppercase', letterSpacing: .45 }}>Total estimado</Typography><Typography sx={{ fontSize: '.7rem', color: 'text.secondary', mt: .15 }}>{form.quantidade || 1} {form.unidade} × {money(Number(form.precoPorMedida) || 0)}/{form.unidade}</Typography></Box>
