...
const ResumoAtividadeMes = ({ saldoDisponivel = 0, despesasPagas = 0, percentual = 0 }) => {
  ...
  const itens = [
    { label: 'Média/dia', value: money(mediaDiaria), hint: `${dia} dias corridos`, Icon: SpeedRoundedIcon },
    { label: 'Disponível/dia', value: money(disponivelDia), hint: `${diasRestantes} dias restantes`, Icon: TodayRoundedIcon },
    ...
    <Box sx={{ mt: .9 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: .65 }}>
        {itens.map(({ label, value, hint, Icon }) => (
          <Box key={label} sx={{ minWidth: 0, p: .82, borderRadius: '14px', bgcolor: 'background.paper', border: '1px solid rgba(72,45,91,.075)', boxShadow: '0 4px 14px rgba(45,11,94,.03)' }}>
