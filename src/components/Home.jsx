...
const dataCurta = (data) => new Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: 'short' }).format(data).replace('.', '');
const diaSemanaCurto = (data) => new Intl.DateTimeFormat('pt-BR', { weekday: 'short' }).format(data).replace('.', '').replace(/^./, c => c.toUpperCase());
const diaSemanaLongo = (data) => new Intl.DateTimeFormat('pt-BR', { weekday: 'long' }).format(data).replace(/^./, c => c.toUpperCase());
const dataRecebimentoLonga = (data) => new Intl.DateTimeFormat('pt-BR', { day: 'numeric', month: 'long' }).format(data);
const textoPrazo = (diff) => diff < 0 ? `${Math.abs(diff)} dia${Math.abs(diff) !== 1 ? 's' : ''} em atraso` : diff === 0 ? 'hoje' : diff === 1 ? 'amanhã' : `faltam ${diff} dias`;
...
const TelaPerfil = ({
  ...
  return (
    <Dialog open={open} onClose={onClose} fullScreen TransitionComponent={Slide}
      ...
      <Box sx={{
        px: { xs: 1.5, sm: 2.5 }, pt: 'calc(12px + var(--app-safe-top))', pb: 1.5,
  ...
  return (
    <Box sx={{ minHeight: 'calc(100dvh - 62px - var(--app-safe-bottom))', bgcolor: 'background.default' }}>
      ...
      <Box sx={{ px: { xs: 1.5, sm: 2 }, pt: { xs: 1.05, sm: 1.5 }, pb: .7, maxWidth: 480, margin: 'auto' }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: .85, animation: 'fadeDown .3s ease both', '@keyframes fadeDown': { '0%': { opacity: 0, transform: 'translateY(-6px)' }, '100%': { opacity: 1, transform: 'translateY(0)' } } }}>
          <Box sx={{ minWidth: 0, pr: .7 }}>
            ...
            <Typography sx={{ fontWeight: 900, fontSize: { xs: '1.28rem', sm: '1.4rem' }, color: 'text.primary', lineHeight: 1.12, letterSpacing: '-.55px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: { xs: 105, sm: 210 } }}>{primeiroNome}</Typography>
          </Box>

          <Box sx={{ display: 'flex', gap: .45, alignItems: 'center' }}>
            {proximoRecebimento && (() => {
              const urgente = proximoRecebimento.diff <= 3;
              return <Box aria-label={`Próximo recebimento: ${dataCurta(proximoRecebimento.data)}, ${textoPrazo(proximoRecebimento.diff)}`} sx={{ minHeight: 38, px: .65, py: .35, borderRadius: '12px', bgcolor: urgente ? 'rgba(247,37,133,.07)' : 'rgba(123,44,191,.06)', border: `1px solid ${urgente ? 'rgba(247,37,133,.18)' : 'rgba(123,44,191,.13)'}`, display: 'flex', alignItems: 'center', gap: .4 }}>
                <CalendarMonthRoundedIcon sx={{ display: { xs: 'none', sm: 'block' }, fontSize: 16, color: urgente ? 'secondary.main' : 'primary.main' }} />
                <Box sx={{ minWidth: 0 }}><Typography sx={{ fontSize: '.55rem', lineHeight: 1, color: 'text.secondary', fontWeight: 900 }}>PRÓX. RECEBIMENTO</Typography><Typography sx={{ fontSize: '.65rem', lineHeight: 1.15, color: urgente ? 'secondary.dark' : 'primary.dark', fontWeight: 900, whiteSpace: 'nowrap', mt: .2 }}>{dataCurta(proximoRecebimento.data)} · {diaSemanaCurto(proximoRecebimento.data)}</Typography><Typography sx={{ fontSize: '.57rem', lineHeight: 1.1, color: 'text.secondary', whiteSpace: 'nowrap' }}>{textoPrazo(proximoRecebimento.diff)}</Typography></Box>
              </Box>;
            })()}
        ...
        <ResumoAtividadeMes
          ...
          diasAteReceber={diasParaPagamento}
        />
            ...
            <Box sx={{ display: 'flex', gap: .6, mt: .8, overflow: 'hidden' }}>
              {alertas.slice(0, 2).map((a, i) => (
                <Box key={`${a.nome}-${i}`} sx={{ flex: 1, minWidth: 0, px: .8, py: .65, borderRadius: '11px', bgcolor: a.atrasado ? 'rgba(229,72,98,.07)' : 'rgba(123,44,191,.055)', border: `1px solid ${a.atrasado ? 'rgba(229,72,98,.15)' : 'rgba(123,44,191,.11)'}` }}>
                  ...
                  <Typography sx={{ fontSize: '.65rem', color: a.atrasado ? 'error.main' : 'text.secondary', mt: .15 }}>{money(a.valor)}</Typography><Typography sx={{ fontSize: '.61rem', color: a.atrasado ? 'error.main' : 'text.secondary', mt: .1 }}>{diaSemanaCurto(a.dataVencimento)}, {dataCurta(a.dataVencimento)} · {textoPrazo(a.diff)}</Typography>
                </Box>
              ))}
            </Box>
          )}
      ...
      <Dialog open={modalAlertas} onClose={() => setModalAlertas(false)} fullWidth maxWidth="xs"
        PaperProps={{ sx: { borderRadius: '20px', m: 1.5, width: 'calc(100% - 24px)', maxHeight: '75dvh' } }}>
        <DialogTitle sx={{ fontWeight: 900, display: 'flex', alignItems: 'center', gap: .7, pb: .7 }}><NotificationsRoundedIcon color="primary" /> Alertas e recebimento</DialogTitle>
        <DialogContent sx={{ pt: '.5rem !important', pb: 1.2 }}>
          {proximoRecebimento && <Box sx={{ p: 1.2, mb: 1.2, borderRadius: '14px', bgcolor: 'rgba(123,44,191,.06)', border: '1px solid rgba(123,44,191,.12)' }}>
            <Typography sx={{ fontSize: '.62rem', fontWeight: 900, color: 'primary.main', letterSpacing: '.7px' }}>PRÓXIMO RECEBIMENTO</Typography>
            <Typography sx={{ fontWeight: 900, mt: .25 }}>Você recebe em {dataRecebimentoLonga(proximoRecebimento.data)}</Typography>
            <Typography sx={{ fontSize: '.78rem', color: 'text.secondary' }}>{diaSemanaLongo(proximoRecebimento.data)} · {textoPrazo(proximoRecebimento.diff)}</Typography>
          </Box>}
          {alertas.length === 0 ? (
            <Box sx={{ textAlign: 'center', py: 1.3 }}>
              <CheckCircleRoundedIcon sx={{ color: 'success.main', fontSize: 30, mb: .4 }} />
              <Typography sx={{ fontWeight: 900, color: 'success.main' }}>Tudo em dia</Typography>
              <Typography sx={{ fontSize: '.76rem', color: 'text.secondary', mt: .2 }}>Nenhuma conta próxima do vencimento.</Typography>
            </Box>
          ) : ['Hoje', 'Próximos 7 dias', 'Mais tarde'].map(grupo => {
            const itens = alertas.filter(a => grupo === 'Hoje' ? a.diff <= 0 : grupo === 'Próximos 7 dias' ? a.diff > 0 && a.diff <= 7 : a.diff > 7);
            if (!itens.length) return null;
            return <Box key={grupo} sx={{ mb: 1.1 }}><Typography sx={{ fontSize: '.64rem', fontWeight: 900, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: '.6px', mb: .5 }}>{grupo}</Typography>{itens.map((a, i) => (
              <Box key={`${grupo}-${a.nome}-${i}`} sx={{ mb: .6, p: 1, borderRadius: '12px', border: '1px solid', borderColor: a.atrasado ? 'rgba(229,72,98,.22)' : 'divider', bgcolor: a.atrasado ? 'rgba(229,72,98,.045)' : 'background.paper' }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 1 }}><Typography sx={{ fontWeight: 900, fontSize: '.82rem', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.nome}</Typography><Typography sx={{ fontWeight: 900, fontSize: '.8rem', flexShrink: 0 }}>{money(a.valor)}</Typography></Box>
                <Typography sx={{ fontSize: '.7rem', color: a.atrasado ? 'error.main' : 'text.secondary', mt: .2 }}>{diaSemanaCurto(a.dataVencimento)}, {dataCurta(a.dataVencimento)} · {textoPrazo(a.diff)}</Typography>
              </Box>
            ))}</Box>;
          })}
        </DialogContent>
        <DialogActions sx={{ px: 2, pb: 1.5, pt: .3 }}><Button fullWidth variant="contained" onClick={() => setModalAlertas(false)}>Fechar</Button></DialogActions>
      </Dialog>
    </Box>
  );
};
