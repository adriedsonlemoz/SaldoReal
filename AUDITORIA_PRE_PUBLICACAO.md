# Auditoria de pré-publicação — Saldo Real 1.0.0-beta.9
...
- Versionamento Android preparado para `versionCode 12` e `versionName 1.0.0-beta.9`.
...
- Revisão beta.9 de responsividade, autocompletes locais e modais sem alteração do schema financeiro.
...
## Validação executada nesta preparação da beta.9
...
- teste isolado do script Android em uma árvore Android de fixture: `app_name` e `title_activity_main` foram sincronizados para **Saldo Real**, `versionCode` para **12**, `versionName` para **1.0.0-beta.9`, e a validação `values` / `values-v27` / `values-v29` passou.
